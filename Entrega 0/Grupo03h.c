...
struct Datos leerNumeros() {
  ...
  do {
    ...
    while (i < sizeof(input) && isdigit(input[i])) {
...
int aproximarLinea(struct NumerosQ m, struct NumerosQ b){ 
  /*
    Esta funcion no la terminamos de implementar, el objetivo era tomar una pendiente y ordenada y N valores
    de x para poder graficar tanto la recta real como los puntos obtenidos tras la conversion usando octave
  */
  int cant = 0;
